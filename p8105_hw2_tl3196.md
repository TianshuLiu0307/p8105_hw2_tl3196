...
```
    ...
    ## # … with 514 more rows, 6 more variables: cigarette_butts <dbl>,
...
Mr and Professor Trash Wheel have collected `1585.2` tons of trash in
total.

So far, Professor Trash Wheel has collected `135.5` tons of trash.
...
After inner joining, the joined dataset contains `786` records from
`1950` to `2015`. The data includes `11` variables, they are
