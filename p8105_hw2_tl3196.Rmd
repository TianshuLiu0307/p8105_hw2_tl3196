...
unemploy_df = read_csv(
  ...
  pivot_longer("1":"12", names_to = "month", values_to = "perc_of_unem", names_transform = list(month = as.numeric)) %>%
  arrange(year, month) %>%
  mutate(month = month.name[.$month]) 
...
After inner joining, the joined dataset contains ``r nrow(join_df)`` records from ``r min(join_df$year)`` to ``r max(join_df$year)``.
